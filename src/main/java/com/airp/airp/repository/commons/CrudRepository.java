package com.airp.airp.repository.commons;


import com.airp.airp.domain.AbstractEntity;
import com.airp.airp.exception.configuration.EntiteNonConnueException;
...
public interface CrudRepository<T extends AbstractEntity> {
	...
	default TypedQuery<T> listerQueryImpl(QueryOptions<T>... queryOptions) {
		...
		optionsOpt.ifPresent(options -> {
            options.applyFetchOperations(root);
        });
